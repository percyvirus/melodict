...
def _extract_raw_skyline(midi_data: pretty_midi.PrettyMIDI, max_duration: float) -> tuple[np.ndarray, np.ndarray]:
    """Helper function to extract the highest pitch trajectory (Raw Skyline) from MIDI data."""
    time_grid = np.arange(0, max_duration, TIME_STEP_SEC)
    ...
    for instrument in midi_data.instruments:
        ...
        for note in instrument.notes:
            ...
            start_idx = int(note.start / TIME_STEP_SEC)
            end_idx = int(end_time / TIME_STEP_SEC)
    ...
    return skyline_pitches, time_grid
...
def load_ground_truth_smart_skyline(
    ...
    skyline_pitches, time_grid = _extract_raw_skyline(midi_data, max_duration)
    
    # Calculate rolling maximum envelope to act as a melodic ceiling
    window_size_frames = int(window_sec / TIME_STEP_SEC)
    melodic_ceiling = maximum_filter1d(skyline_pitches, size=window_size_frames, mode='constant', cval=0.0)
    
    # Filter drops that fall significantly below the ceiling (harmony bleeding)
    ...
    # Convert remaining valid MIDI pitches to Hz
...
def predict_realtime(
    ...
    est_freqs_arr = np.array(est_freqs)
    
    # Apply light temporal hysteresis to remove isolated 1-frame jitters
    if len(est_freqs_arr) >= 5:
        est_freqs_arr = medfilt(est_freqs_arr, kernel_size=5)
        
    return np.array(est_times), est_freqs_arr
...
def predict_offline_upper_bound(audio_path: Path, max_duration: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    # Imported locally to ensure ONNX environment variables are set by the EngineFactory first
    ...
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as temp_wav:
        ...
        # Suppress heavy TensorFlow/Basic-Pitch logging
        with open(os.devnull, 'w') as fnull, redirect_stdout(fnull), redirect_stderr(fnull):
            _, midi_data, _ = predict(temp_wav.name)
        
    # Re-use the Skyline logic for the model's output
...
def main() -> None:
    ...
    parser.add_argument("--disable_offline", action="store_true", help="Skip the Offline Upper Bound calculation.")
    ...
    try:
        ...
        print(f"Error parsing hyperparameters: {e}. Ensure they are comma-separated numbers.")
        return
    
    engine = EngineFactory.create("basic-pitch")
    if not engine:
        print("Error: Could not load the basic-pitch engine.")
        return
    ...
    print(f"Offline condition (Ceiling) is {'DISABLED' if args.disable_offline else 'ENABLED'}.")
    ...
    for audio_path in tqdm(audio_files, desc="Processing Files"):
        ...
        # 1. Offline Upper Bound (Full File)
